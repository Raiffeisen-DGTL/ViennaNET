...
namespace ViennaNET.Messaging.Tests.Unit.Factories
{
  ...
  public class QueueReactorFactoryTests
  {
    public static readonly object[] CreateQueueReactor_AdapterType_CorrectReactorReturned_Source = {
      new object[]
      {
        Given.MessageAdapter
          .WithQueueConfiguration(new KafkaQueueConfiguration {ProcessingType = MessageProcessingType.ThreadStrategy, IntervalPollingQueue = 1000})
      ...
      {
        Given.MessageAdapter
          .WithQueueConfiguration(new KafkaQueueConfiguration {ProcessingType = MessageProcessingType.Subscribe, IntervalPollingQueue = 1000})
      ...
      {
        Given.MessageAdapter
          .WithQueueConfiguration(new KafkaQueueConfiguration {ProcessingType = MessageProcessingType.SubscribeAndReply, IntervalPollingQueue = 1000})
    ...
    [Test]
    public void CreateQueueReactor_ProcessorNotSupportsProcessingType_ThrowsException()
    {
      const string queueId = "ReName";
      var messageProcessor = Mock.Of<IMessageProcessor>();
      ...
      var ex = Assert.Throws<MessagingException>(() => queueReactorFactory.CreateQueueReactor(queueId));
      StringAssert.Contains("Processing type", ex.Message);
    }
  }
}