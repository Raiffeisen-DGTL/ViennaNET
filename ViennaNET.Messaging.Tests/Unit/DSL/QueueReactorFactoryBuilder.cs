...
namespace ViennaNET.Messaging.Tests.Unit.DSL
{
  internal class QueueReactorFactoryBuilder
  {
    ...
    private readonly LinkedList<MessageProcessorData> _messageProcessors = new LinkedList<MessageProcessorData>();  
    ...
    private readonly LinkedList<MessageProcessorAsyncData> _messageProcessorsAsync = new LinkedList<MessageProcessorAsyncData>();

    private IMessageAdapter _messageAdapter;

    public QueueReactorFactoryBuilder WithMessageProcessor(IMessageProcessor messageProcessor, string queueNameToRegister = null)
    {
      _messageProcessors.AddLast(new MessageProcessorData { messageProcessor = messageProcessor, queueName = queueNameToRegister });
      return this;
    } 
    
    public QueueReactorFactoryBuilder WithMessageProcessorAsync(IMessageProcessorAsync messageProcessor, string queueNameToRegister = null)
    {
      _messageProcessorsAsync.AddLast(new MessageProcessorAsyncData { messageProcessor = messageProcessor, queueName = queueNameToRegister });
      return this;
    }
