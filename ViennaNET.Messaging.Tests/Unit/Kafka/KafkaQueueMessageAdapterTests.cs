...
using ViennaNET.Messaging.Tests.Unit.Kafka.DSL;
  ...
  public class KafkaQueueMessageAdapterTests
  {
    ...
    public void Connect_Consumer_CreateConsumerCalled()
    {
      ...
      connectionFactoryMock.Verify(x
        => x.CreateConsumer(config, It.IsAny<Action<IConsumer<Ignore, byte[]>, LogMessage>>(), It.IsAny<Action<IConsumer<Ignore, byte[]>, Error>>()));
    ...
    public void Connect_Producer_CreateProducerCalled()
    {
      ...
      connectionFactoryMock.Verify(x
        => x.CreateProducer(config, It.IsAny<Action<IProducer<Null, byte[]>, LogMessage>>(), It.IsAny<Action<IProducer<Null, byte[]>, Error>>()));
      Assert.IsTrue(adapter.IsConnected);
    }
    ...
    public void Send_Message_ProduceCalled()
    {
      ...
      connectionFactoryMock.ProducerMock.Verify(
        x => x.Produce(config.QueueName, It.IsAny<Message<Null, byte[]>>(), It.IsAny<Action<DeliveryReport<Null, byte[]>>>()));
    }
    ...
    public void Receive_EmptyQueue_ThrowsException()
    {
      ...
      connectionFactoryMock.ConsumerMock.Verify(x => x.Consume(It.IsAny<CancellationToken>()));
    } 
