...
namespace ViennaNET.Logging.Log4NetImpl
{
  ...
  public class CustomRollingFileAppender : FileAppender
  {
    ...
    private void InitializeFromOneFile(string baseFile, string curFileName)
    {
      ...
      {
        string date = m_dateTime.Now.ToString(m_datePattern, DateTimeFormatInfo.InvariantInfo);
    ...
    private RollPoint ComputeCheckPeriod(string datePattern)
    {
      ...
      for (int i = (int)RollPoint.TopOfMinute; i <= (int)RollPoint.TopOfMonth; i++)
      {
        ...
        string r1 = NextCheckDate(s_date1970, (RollPoint)i)
        ...
        else
        {
          ...
          if (m_curSizeRollBackups >= m_maxSizeRollBackups && m_maxSizeRollBackups > 0)
          {
            ...
            if (!m_staticLogFileName)
            {
              if (m_preserveLogFileNameExtension)
              {
                int separationDotIndex = archiveFileBaseName.LastIndexOf('.', archiveFileBaseName.LastIndexOf('.') - 1);
