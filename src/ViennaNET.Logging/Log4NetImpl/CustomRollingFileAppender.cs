...
namespace ViennaNET.Logging.Log4NetImpl
{
  ...
  public class CustomRollingFileAppender : FileAppender
  {
    ...
    /// </summary>
    /// <value>
    ///   An implementation of the <see cref="RollingFileAppender.IDateTime" /> interface which returns the current date and
    ///   time.
    /// </value>
    /// <remarks>
    ///   <para>
          ...
          if (LogLog.IsErrorEnabled)
          {
            ...
            if (m_maxSizeRollBackups != 0 && FileExists(fileName))
            {
              LogLog.Error(declaringType,
                "CustomRollingFileAppender: INTERNAL ERROR. Append is False but OutputFile [" + fileName +
                "] already exists.");
            }
          }
        }
    ...
    private void InitializeFromOneFile(string baseFile, string curFileName)
    {
      ...
      try
      {
        ...
        if (backup > m_curSizeRollBackups)
        {
          ...
          LogLog.Debug(declaringType,
            "File name [" + curFileName + "] moves current count to [" + m_curSizeRollBackups + "]");
        }
      }
    ...
    private RollPoint ComputeCheckPeriod(string datePattern)
    {
      ...
      for (var i = (int)RollPoint.TopOfMinute; i <= (int)RollPoint.TopOfMonth; i++)
      {
        ...
        var r1 = NextCheckDate(s_date1970, (RollPoint)i)
          .ToString(datePattern, DateTimeFormatInfo.InvariantInfo);
    ...
    protected void DeleteFile(string fileName)
    {
      if (FileExists(fileName))
      {
        ...
        {
          LogLog.Debug(declaringType,
            "Exception while moving file to be deleted [" + fileName + "] -> [" + tempFileName + "]", moveEx);
        }
    ...
    /// <summary>
    ///   Implementation of <see cref="RollingFileAppender.IDateTime" /> that returns the current time as the coordinated
    ///   universal time (UTC).
    /// </summary>
