...
namespace ViennaNET.Logging.Log4NetImpl
{
  ...
  /// <remarks>
  /// <para>
  /// CustomRollingFileAppender can roll log files based on size or date or both
  /// depending on the setting of the <see cref="RollingStyle"/> property.
  /// When set to <see cref="RollingFileAppender.RollingMode.Size"/> the log file will be rolled
  /// once its size exceeds the <see cref="MaximumFileSize"/>.
  /// When set to <see cref="RollingFileAppender.RollingMode.Date"/> the log file will be rolled
  /// once the date boundary specified in the <see cref="DatePattern"/> property
  /// is crossed.
  /// When set to <see cref="RollingFileAppender.RollingMode.Composite"/> the log file will be
  /// rolled once the date boundary specified in the <see cref="DatePattern"/> property
  /// is crossed, but within a date boundary the file will also be rolled
  /// once its size exceeds the <see cref="MaximumFileSize"/>.
  /// When set to <see cref="RollingFileAppender.RollingMode.Once"/> the log file will be rolled when
  ...
  /// 
  /// <para>
  /// The <see cref="CustomRollingFileAppender"/> extends the <see cref="FileAppender"/> and
  ...
  /// </note>
  /// </remarks>
  public class CustomRollingFileAppender : FileAppender
  {
    ...
    /// <summary>
    /// The fully qualified type of the CustomRollingFileAppender class.
    /// </summary>
    /// <remarks>
    ...
    {
      get => m_maxFileSize.ToString(NumberFormatInfo.InvariantInfo);
    ...
    /// <remarks>
    /// <para>
    /// The default rolling style is <see cref="RollingFileAppender.RollingMode.Composite" />.
    /// </para>
    /// <para>
    /// When set to <see cref="RollingFileAppender.RollingMode.Once"/> this appender's
    ...
    {
      get => m_preserveLogFileNameExtension;
    ...
    /// <remarks>
    /// <para>
    /// Handles append time behavior for CustomRollingFileAppender.  This checks
          ...
          if (LogLog.IsErrorEnabled)
          {
            ...
            if (m_maxSizeRollBackups != 0 && FileExists(fileName))
            {
              LogLog.Error(declaringType,
                "CustomRollingFileAppender: INTERNAL ERROR. Append is False but OutputFile [" + fileName + "] already exists.");
            }
          }
        }
      ...
      if (m_staticLogFileName && m_rollDate)
      {
        if (FileExists(m_baseFileName))
        {
          ...
          LogLog.Debug(declaringType,
            "[" + last.ToString(m_datePattern, DateTimeFormatInfo.InvariantInfo) + "] vs. ["
            + m_now.ToString(m_datePattern, DateTimeFormatInfo.InvariantInfo) + "]");

          if (!(last.ToString(m_datePattern, DateTimeFormatInfo.InvariantInfo)
                    .Equals(m_now.ToString(m_datePattern, DateTimeFormatInfo.InvariantInfo))))
          {
            m_scheduledFilename = CombinePath(m_baseFileName,
              last.ToString(m_datePattern, DateTimeFormatInfo.InvariantInfo));
    ...
    protected void ExistingInit()
    {
      ...
      if (AppendToFile == false)
      {
        ...
        if (fileExists)
        {
          ...
          {
            LogLog.Debug(declaringType,
              "Output file [" + fileName + "] already exists. MaxSizeRollBackups is 0; cannot roll. Overwriting existing file.");
          }
          else
          {
            LogLog.Debug(declaringType,
              "Output file [" + fileName + "] already exists. Not appending to file. Rolling existing file out of the way.");
    ...
    private void InitializeFromOneFile(string baseFile, string curFileName)
    {
      if (baseFile == null)
      {
        throw new LogException("Parameter baseFile is null");
      }

      if (curFileName == null)
      {
        throw new LogException("Parameter curFileName is null");
      }
      ...
      {
        var date = m_dateTime.Now.ToString(m_datePattern, DateTimeFormatInfo.InvariantInfo);
    ...
    private int GetBackUpIndex(string curFileName)
    {
      if (curFileName == null)
      {
        throw new LogException("curFileName");
      }
      var backUpIndex = -1;
    ...
    private void InitializeRollBackups(string baseFile, ArrayList arrayFiles)
    {
      if (null != arrayFiles)
      {
        var baseFileLower = baseFile.ToLower(System.Globalization.CultureInfo.InvariantCulture);
    ...
    private RollPoint ComputeCheckPeriod(string datePattern)
    {
      ...
      var r0 = s_date1970.ToString(datePattern, System.Globalization.DateTimeFormatInfo.InvariantInfo);
      ...
      for (var i = (int)RollPoint.TopOfMinute; i <= (int)RollPoint.TopOfMonth; i++)
      {
        ...
        var r1 = NextCheckDate(s_date1970, (RollPoint)i)
          .ToString(datePattern, System.Globalization.DateTimeFormatInfo.InvariantInfo);
    ...
    public override void ActivateOptions()
    {
      ...
      m_mutexForRolling = new Mutex(false, m_baseFileName.Replace("\\", "_")
                                                         .Replace(":", "_")
                                                         .Replace("/", "_"));
    ...
    protected void RollOverTime(bool fileIsOpen)
    {
      if (m_staticLogFileName)
      {
        ...
        var dateFormat = m_now.ToString(m_datePattern, System.Globalization.DateTimeFormatInfo.InvariantInfo);
      ...
      if (m_maxSizeRollBackups != 0)
      {
        ...
        else
        {
          ...
          if (m_curSizeRollBackups >= m_maxSizeRollBackups && m_maxSizeRollBackups > 0)
          {
            ...
            // If we have a preserveFileNameExtension set to true, we must recreate basefilename from existing filename
            var archiveFileBaseName = baseFileName;
            if (!m_staticLogFileName)
            {
              if (m_preserveLogFileNameExtension)
              {
                var separationDotIndex = archiveFileBaseName.LastIndexOf('.', archiveFileBaseName.LastIndexOf('.') - 1);
                if (separationDotIndex >= 0)
                {
                  archiveFileBaseName = archiveFileBaseName.Substring(0, separationDotIndex)
    ...
    /// <summary>
    /// Default implementation of <see cref="RollingFileAppender.IDateTime"/> that returns the current time.
    /// </summary>
    ...
    /// <summary>
    /// Implementation of <see cref="RollingFileAppender.IDateTime"/> that returns the current time as the coordinated universal time (UTC).
    /// </summary>
