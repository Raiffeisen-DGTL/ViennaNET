...
{
  /// <summary>
  ///   Исключение, возникающее в случае если для события предметной области не определен преобразователь на интеграционное
  ///   событие.
  /// </summary>
  public class IntegrationEventMapperNotFoundException : Exception
  {
    ...
    public IntegrationEventMapperNotFoundException(Type integrationEventType, Type eventType) :
      base(
        $"Concrete integration event mapper from event {eventType.FullName} to integration event {integrationEventType.FullName} not found")
    {
    }
  }
}