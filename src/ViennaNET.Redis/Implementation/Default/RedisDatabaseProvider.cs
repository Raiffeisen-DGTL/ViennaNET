...
namespace ViennaNET.Redis.Implementation.Default
{
  ...
  public class RedisDatabaseProvider : IRedisDatabaseProvider
  {
    ...
    public RedisDatabaseProvider(IConnectionConfiguration configuration,
    ...
    private ConnectionMultiplexer CreateConnection()
    {
      ...
      connection.ConnectionRestored += (s, e) =>
        _logger.LogDebug("Connection to Redis has been restored. ConnectionType = {connectionType}. "
                         + "EndPoint = {endPoint}. FailureType = {failureType}", e.ConnectionType, e.EndPoint,
          e.FailureType);
      connection.ErrorMessage += (s, e) =>
        _logger.LogError(e.Message, "Error has been received. Message = {message}. EndPoint = {endPoint}.", e.Message,
          e.EndPoint);
