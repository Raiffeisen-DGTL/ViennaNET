...
namespace ViennaNET.Messaging.RabbitMQQueue.Tests
{
  ...
  public class RabbitMQQueueMessageAdapterTests
  {
    ...
    //  public void Connect_VariousExchangeAndQueueNames_InitializeBindsExpectedCalled(string exchange, string queue,
    //    bool isBindCalled)
    //  {
    ...
    //    busFactory.AdvancedBusMock.Verify(
    //      x => x.Bind(It.IsAny<IExchange>(), It.IsAny<IQueue>(), It.Is<string>(key => key == routings[0])),
    //      Times.AtLeastOnce);
    //    busFactory.AdvancedBusMock.Verify(
    //      x => x.Bind(It.IsAny<IExchange>(), It.IsAny<IQueue>(), It.Is<string>(key => key == routings[1])),
    //      Times.AtLeastOnce);
    //  }
    ...
    //  busFactory.AdvancedBusMock.Verify(
    //    x => x.Get(It.IsAny<IQueue>()));
    //    //x => x.CreatePullingConsumer(It.IsAny<IQueue>()).PullAsync().GetAwaiter().GetResult());
    //}
  }
}