...
namespace ViennaNET.Orm.Factories
{
  ...
  public class SessionFactoryProvidersManager : ISessionFactoryProvidersManager
  {
    ...
    /// <summary>
    ///   Инициализирует экземпляр ссылками на <see cref="IConfiguration" /> и коллекцию
    ///   <see cref="ISessionFactoryProviderGetter" />
    /// </summary>
    ...
    public SessionFactoryProvidersManager(IConfiguration configuration,
      IList<ISessionFactoryProviderGetter> providerGetters)
    {
      _providers =
        new Lazy<IEnumerable<ISessionFactoryProvider>>(() =>
          GetSessionFactoryProvidersInternal(configuration, providerGetters));
    }
      ...
      IConfiguration configuration, IList<ISessionFactoryProviderGetter> providerGetters)
    {
      ...
      {
        throw new SessionFactoryProviderException(
      ...
      foreach (var connectionInfo in connectionInfos)
      {
        ...
        {
          throw new SessionFactoryProviderException(
            $"Session factory provider getter with nick = {connectionInfo.Nick} is not found");
        }
