namespace ViennaNET.Validation.Rules.ValidationResults.RuleMessages
{
  ...
  public class WarningRuleMessage : BaseRuleMessage
  {
    ...
    public WarningRuleMessage(MessageIdentity identity, string error, params object[] args) : base(null, identity, null,
      error, args)
    {
    }
    ...
    /// <param name="error">Текст ошибки</param>
    /// <param name="args">Аргументы для форматирования текста</param>
    public WarningRuleMessage(object state, string customCode, MessageIdentity identity, string error,
      params object[] args) :
