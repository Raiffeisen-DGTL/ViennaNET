...
using Document = ViennaNET.Word.Document;

namespace Tests
{
    [TestFixture(Category = "Unit", TestOf = typeof(Document))]
    public class DocumentTests
    {
        ...
        [TestCase(" ", false, ExpectedResult = null,
            Description =
                "Проверяет что не возможно изменить значение, если имя стандартного элемента состоит из пробела.")]
        [TestCase("     ", true, ExpectedResult = null,
            Description =
                "Проверяет что не возможно изменить значение, если имя стандартного элемента состоит из пробелов.")]
            ...
            Assert.That(() =>
                {
                    ...
                    actual = body
                        ...
                        .Single(element => element.SdtProperties.GetFirstChild<SdtAlias>().Val == name)
                        .Descendants<Text>()
                        .First()
                        .Text;
                },
                Throws.Nothing | Throws.InstanceOf<ArgumentNullException>() |
                Throws.InstanceOf<InvalidSdtElementException>());
        ...
        [TestCase("", " ", ExpectedResult = null,
            Description =
                "Проверяет что не возможно вставить текст в документ, если имя стандартного элемента пустая строка.")]
        [TestCase(" ", "test", ExpectedResult = null,
            Description =
                "Проверяет что не возможно вставить текст в документ, если имя стандартного элемента состоит из пробелов.")]
        [TestCase(null, "    ", ExpectedResult = null,
            Description =
                "Проверяет что не возможно вставить текст в документ, если имя стандартного элемента не определено.")]
        [Test]
            ...
            Assert.That(() =>
                {
                    ...
                    actual = body?.Descendants<SdtElement>()
                        .SingleOrDefault(element => element.SdtProperties.GetFirstChild<SdtAlias>()?.Val == name)
                        ?.Descendants<Text>().First().Text;
                },
                Throws.Nothing | Throws.InstanceOf<ArgumentNullException>() |
                Throws.InstanceOf<InvalidSdtElementException>());
        ...
        [TestCase("Date", "12.22.1993", ExpectedResult = "22.12.1993",
            Description =
                "Вставка даты в формате (mm.dd.yyyy) в документ, учитывая формат заданный на уровне разметки в документе.")]
        [TestCase("Date", "12/22/1993", ExpectedResult = "22.12.1993",
            Description =
                "Вставка даты в формате (mm/dd/yyyy) в документ, учитывая формат даты заданный на уровне разметки в документе.")]
        [TestCase("", "01.01.0001", ExpectedResult = null,
            Description =
                "Проверяет что не возможно вставить дату в документ, если имя стандартного элемента пустая строка.")]
        [TestCase(" ", "01.01.0001", ExpectedResult = null,
            Description =
                "Проверяет что не возможно вставить дату в документ, если имя стандартного элемента состоит из пробелов.")]
        [TestCase(null, "01.01.0001", ExpectedResult = null,
            Description =
                "Проверяет что не возможно вставить дату в документ, если имя стандартного элемента не определено.")]
        [Test]
            ...
            Assert.That(() =>
                {
                    document.Fill(wordPackage.MainDocumentPart);
                    actual = body?.Descendants<SdtElement>()
                        .Single(block => block.Descendants<SdtAlias>().First().Val == name)
                        .Descendants<Text>()
                        .First()
                        .Text;
                },
        ...
        [TestCase("",
            Description =
                "Проверяет что не возможно вставить изображение в документ, если имя стандартного элемента пустая строка.")]
        [TestCase(" ",
            Description =
                "Проверяет что не возможно вставить изображение в документ, если имя стандартного элемента состоит из пробелов.")]
        [TestCase(null,
            Description =
                "Проверяет что не возможно вставить изображение в документ, если имя стандартного элемента не определено.")]
        [Test]
                ...
                try
                {
                    ...
                    if (name == "Image")
                    {
                        var blip = body?.Descendants<SdtAlias>()
                            .FirstOrDefault(alias => alias.Val == name)?.Parent?.Parent?.Descendants<Blip>()
                            .SingleOrDefault();
                        ...
                        Assert.That(
                            imagePart.ContentType, Is.EqualTo("image/png"),
                            "В тестовом документе должен быть объявлен элемент ImagePart с ContentType=image/png");

                        using var sourceStreamReader =
                            new StreamReader(new MemoryStream(Convert.FromBase64String(Resources.TestImageData)));
        ...
        public void Fill(string? name, int id, string? firstName, bool gender)
        {
            ...
            var document = new TableTestDocument(name!, new List<object> { row });
        ...
        [TestCase("", " ", ExpectedResult = null,
            Description =
                "Проверяет что не возможно вставить текст в закладку в документе, если имя стандартного элемента пустая строка.")]
        [TestCase(" ", "test", ExpectedResult = null,
            Description =
                "Проверяет что не возможно вставить текст в закладку в документе, если имя стандартного элемента состоит из пробелов.")]
        [TestCase(null, "    ", ExpectedResult = null,
            Description =
                "Проверяет что не возможно вставить текст в закладку в документе, если имя стандартного элемента не определено.")]
        [Test]
