...
namespace ViennaNET.Validation.Tests
{
    ...
    public class RuleValidatorTests
    {
        ...
        public void Validator_ManyContexts_CorrectResult()
        {
            ...
            var res = validator.ValidateMany(
                new IValidationRuleSet<IMainEntity>[] { validationContext, validationWhenContext },
                entity.Object, null!);

            Assert.That(res.Results, Has.Count.EqualTo(2));
        }
        ...
        public void Validator_FluentRule_HasManyMessages()
        {
            ...
            Assert.Multiple(() =>
            {
                ...
                Assert.That(res.Results[0].Messages, Has.Count.EqualTo(2));
            });
        }
        ...
        public void Validator_FluentRule_OverrideMessages()
        {
            ...
            Assert.Multiple(() =>
            {
                Assert.That(res.Results[0].Messages.First().Error,
                    Is.EqualTo("A type of an account 1234567890 must be CurrentInThirdpartyBank"));
                Assert.That(res.Results[0].Messages.Last().Error, Is.EqualTo("A type of an action must not be Update"));
            });
        }
        ...
        public void Validator_RuleDependOnContext_GenerateError()
        {
            ...
            Assert.That(res.Results.Any, Is.True);
        }
        ...
        public void UseRuleFluentRule_WhenAggregatedPropertyHasNull_MustReturnsError()
        {
            ...
            accountInfo.Setup(x => x.Account).Returns<string>(null!);
            accountInfo.Setup(x => x.AccountCba).Returns("dfhbv");
            accountInfo.Setup(x => x.AccountType).Returns("dfhbv");
        ...
        public void UseRuleFluentRule_WhenAggregatedPropertyHas2Nulls_MustReturns2ErrorMessages()
        {
            ...
            accountInfo.Setup(x => x.Account).Returns<string>(null!);
            accountInfo.Setup(x => x.AccountCba).Returns("dfhbv");
            accountInfo.Setup(x => x.AccountType).Returns<string>(null!);
            entity.Setup(x => x.AccountsInfo).Returns(accountInfo.Object);
        ...
        private Mock<IDbAccessor> GetDbAccessor()
        {
            ...
            dbAccessor.Setup(a => a.GetContractAccountType(It.IsAny<string>())).Returns("Текущий в банке");
            dbAccessor
                .Setup(a => a.GetContractAccount(It.IsAny<int>(), It.IsAny<string>()))
