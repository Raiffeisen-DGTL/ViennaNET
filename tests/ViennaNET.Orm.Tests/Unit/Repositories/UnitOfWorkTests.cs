...
namespace ViennaNET.Orm.Tests.Unit.Repositories
{
  ...
  public class UnitOfWorkTests
  {
    ...
    public void Create_CannotRegisterUoW_ExceptionThrown()
    {
      ...
      Assert.Throws<UowException>(
        () => new UnitOfWork(entityFactoryService.Object, sessionManager.Object, new NullLogger<UnitOfWork>()),
    ...
    public async Task CommitAsync_SimpleCall_SessionManagerMethodCalled()
    {
      ...
      sessionManager.Verify(x => x.CommitAllAsync(default));
    }
    ...
    public async Task SaveAsync_SimpleCall_SessionManagerMethodCalled()
    {
      ...
      sessionManager.Verify(x => x.SaveAllAsync(default));
    }
    ...
    public async Task RollbackAsync_SimpleCall_SessionManagerMethodCalled()
    {
      ...
      sessionManager.Verify(x => x.RollbackAllAsync(null, default));
    }
    ...
    public async Task MarkDirtyAsync_CallsSessionProvider_EvictMethodCalled()
    {
      ...
      Assert.Multiple(() =>
      {
        session.Verify(x => x.EvictAsync(entity, default));
        entityRepository.Verify(x => x.AddAsync(entity, default));
      });
    }
    ...
    public async Task MarkDeletedAsync_CallsSessionProvider_EvictMethodCalled()
    {
      ...
      entityRepository.Verify(x => x.DeleteAsync(entity, default));
    }
    ...
    private static UnitOfWork CreateUnitOfWork(
      Mock<ISessionManager> sessionManager, out Mock<ISession> session,
      out Mock<IEntityRepository<TestCommand>> entityRepository)
    {
      ...
      sessionManager.Setup(x => x.RegisterUoW(It.IsAny<IUnitOfWork>()))
        .Returns(true);
      var uow = new UnitOfWork(entityFactoryService.Object, sessionManager.Object, new NullLogger<UnitOfWork>());
      return uow;
    }
  }
}