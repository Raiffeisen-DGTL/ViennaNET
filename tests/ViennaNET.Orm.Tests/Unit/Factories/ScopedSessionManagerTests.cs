...
namespace ViennaNET.Orm.Tests.Unit.Factories
{
  ...
  public class ScopedSessionManagerTests
  {
    ...
    public async Task CommitAllAsync_TransactionInactive_CommitDidNotCall()
    {
      ...
      await Task.WhenAll(manager.CommitAllAsync(default));

      transaction.Verify(x => x.CommitAsync(default), Times.Never);
    }
    ...
    public async Task CommitAllAsync_TransactionActive_CommitCalled()
    {
      ...
      transaction.Verify(x => x.CommitAsync(default), Times.Once);
    }
    ...
    public async Task SaveAllAsync_HasNoSessions_SaveDidNotCall()
    {
      ...
      await Task.WhenAll(manager.SaveAllAsync(default));

      session.Verify(x => x.FlushAsync(default), Times.Never);
    }
    ...
    public async Task SaveAllAsync_HasSessions_SaveCalled()
    {
      ...
      session.Verify(x => x.FlushAsync(default), Times.Once);
    }
    ...
    public async Task RollbackAllAsync_TransactionIsActive_RollbackCalled()
    {
      ...
      await Task.WhenAll(manager.RollbackAllAsync(null, default));

      Assert.Multiple(() =>
      {
        transaction.Verify(x => x.RollbackAsync(default), Times.Once);
        session.Verify(x => x.Dispose(), Times.Never);
      });
    }
    ...
    public async Task RollbackAllAsync_TransactionIsInactive_RollbackDidNotCalled()
    {
      ...
      Assert.Multiple(() =>
      {
        transaction.Verify(x => x.RollbackAsync(default), Times.Never);
        session.Verify(x => x.Dispose(), Times.Never);
      });
    }
    ...
    public async Task RollbackAllAsync_CloseSessionsAndHasException_SessionClearCalled()
    {
      ...
      await Task.WhenAll(manager.RollbackAllAsync(true, default));
    ...
    public void GetStatelessSession_SimpleCall_SessionMethodCalled()
    {
      ...
      var manager = new ScopedSessionManager(sessionFactoryManager.Object, new NullLogger<ScopedSessionManager>());
    ...
    private static ScopedSessionManager GetSessionFactoryManager(out Mock<ISession> session,
