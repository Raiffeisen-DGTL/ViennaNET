...
namespace ViennaNET.Orm.Tests.Unit.Factories
{
  ...
  public class SessionFactoryProvidersManagerTests
  {
    ...
    {
      var configuration = new ConfigurationBuilder().AddJsonFile("TestConfiguration/orm.json", true)
        .Build();
      ...
      var sessionFactoryProvidersManager =
        new SessionFactoryProvidersManager(configuration,
          new[] { db2Getter.Object, msSqlGetter.Object, oracleGetter.Object, postgreSqlGetter.Object });
    ...
    {
      var configuration = new ConfigurationBuilder().AddJsonFile("TestConfiguration/ormNoDBSection.json", true)
        .Build();

      var sessionFactoryProvidersManager =
        new SessionFactoryProvidersManager(configuration, new ISessionFactoryProviderGetter[] { });

      Assert.Throws<SessionFactoryProviderException>(
        () => sessionFactoryProvidersManager.GetSessionFactoryProvider(It.IsAny<string>()),
