...
namespace ViennaNET.Excel.Impl
{
  internal class NPoiWorkSheet : IWorkSheet
  {
    ...
    public object ReadCellByName(string name)
    {
      ...
      var sheet = ExcelFile[(string)cellRef.SheetName];
    ...
    public void CreateVerticalRange(string name, int cellIndex, int startRow, int endRow)
    {
      ...
      forumla.Append(Name)
             .Append("!$")
             .Append((string)CellReference.ConvertNumToColString(cellIndex))
             .Append("$")
             .Append(startRow + 1);
      if (endRow > startRow)
      {
    ...
    public void CreateRange(string name, int startCell, int startRow, int endCell, int endRow)
    {
      ...
      forumla.Append(Name)
             .Append("!$")
             .Append((string)CellReference.ConvertNumToColString(startCell))
             .Append("$")
             .Append(startRow + 1)
             .Append(":$")
             .Append((string)CellReference.ConvertNumToColString(endCell))
             .Append("$")
             .Append(endRow + 1);
      namedRange.RefersToFormula = forumla.ToString();
    }
